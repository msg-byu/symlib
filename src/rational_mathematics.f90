...
!!<summary>Find the Hermite normal form of a a given integer matrix. Similar to the SNF finder 
!!above but a little simpler. This routine is not very elegant, just brute force. Don't be 
!!disappointed.</summary>
!!<parameter name="S">The 3x3 integer matrix describing the relationship between two 
!!commensurate lattices.</parameter>
!!<parameter name="H">The resulting HNF matrix.</parameter>
!!<parameter name="B">The transformation matrix such that $H = SB$.</parameter>
subroutine HermiteNormalForm(S,H,B)
integer, intent(in) :: S(3,3)
integer, intent(out), dimension(3,3) :: H, B

!!<local name="tempcol">When two columns need to be swapped, tempcol mediates the swap.</local>
!!<local name="check">Stores the HNF as a vector; only used as failsafe for meeting HNF criteria.</local>
!!<local name="maxidx, minidx">The indices of max/min values in a column.</local>
!!<local name="multiple">When reducing the columns to HNF form, the multiplicative relationship
!!between two elements of two columns; reused.</local>
!!<local name="minm">The smallest element in row 1 of the HNF being constructed.</local>
...
!!<summary>Swaps the column 'k' with whichever column has the highest value (out of
!!the columns to the right of 'k' in row 'k'). The swap is performed in both matrices
!!'M' and 'B'.</summary> 
!!<parameter name="M">The matrix being transformed.</parameter>
!!<parameter name="B">Usually a matrix to keep track of the transformation steps on 'M'.</parameter>
!!<parameter name="k">The column to swap, as described in summary.</parameter>
...
!!<summary>Finds the indices corresponding the minimum and maximum values in an
!!integer vector.</summary>
