...
! GLWH July 10 2015. This stupid do loop business is needed to circumvent a compiler bug in ifort
  ...
  subroutine matrix_inverse(a, b, err_, eps_)
    ...
    b=1/c*transpose(reshape(avec,(/3,3/)))     
    if (present(err_)) then ! check to see if the identity is generated by matmul(a,b)
      if (.not. equal(matmul(a,b),&
         reshape((/1._dp,0._dp,0._dp,0._dp,1._dp,0._dp,0._dp,0._dp,1._dp/),(/3,3/)),&
         eps,0._dp)) err_=.true.
    endif
